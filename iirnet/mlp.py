...
def construct_filter_zpk(x: torch.Tensor, model_order: int, eps: float = 1e-8):
    # reshape into sos format (n_section, (b0, b1, b2, a0, a1, a2))
    n_sections = model_order // 2
    ...
    pole = (1 - eps) * pole * torch.tanh(pole.abs()) / (pole.abs().clamp(eps))

    # ensure zeros inside unit circle
    zero = torch.complex(zero_real, zero_imag)
    zero = (1 - eps) * zero * torch.tanh(zero.abs()) / (zero.abs().clamp(eps))
...
def construct_filter_sos(x: torch.Tensor, model_order: int, eps: float = 1e-8):
    # reshape into sos format (n_section, (b0, b1, b2, a0, a1, a2))
    n_sections = model_order // 2
    sos = x.view(-1, n_sections, 6)
    ...
    # ensure stability
    a0 = torch.ones(g.shape).type_as(x)
    a1 = 2 * torch.tanh(a1)
...
class MLPModel(torch.nn.Module):
    ...
    def __init__(
        ...
        model_order: int = 2,
        normalization="none",
        output: str = "sos",  # either "sos" or "zpk"
        eps: float = 1e-8,
    ):
        super().__init__()
        ...
        if self.output == "zpk":
            self.construct_filter = construct_filter_zpk
        elif self.output == "sos":
            self.construct_filter = construct_filter_sos
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.construct_filter(x, self.model_order, self.eps)
